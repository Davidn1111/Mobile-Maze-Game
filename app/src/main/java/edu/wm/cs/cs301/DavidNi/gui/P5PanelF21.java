...
public interface P5PanelF21 {
    ...
    public static int getWallColor(int distance, int cc, int extensionX) {
        ...
        final int part1 = (distance/4) & 7;
        final int add = (extensionX != 0) ? 1 : 0;
        final int rgbValue = ((part1 + 2 + add) * 70) / 8 + 80;
        
        // argb value returned
        int argb = 0;
        // Add values to argb using bitshift
        // Set default alpha value to 255
        argb |= 255 << 24;

        // mod used to limit the number of colors to 6
        // Return the corresponding argb values
        switch ((((distance/4) >> 3) ^ cc) % 6) {
        case 0:
            argb |= rgbValue << 16;
            argb |= RGB_DEF << 8;
            argb |= RGB_DEF;
