...
import edu.wm.cs.cs301.DavidNi.generation.BSPBranch;
import edu.wm.cs.cs301.DavidNi.generation.BSPLeaf;
import edu.wm.cs.cs301.DavidNi.generation.BSPNode;
import edu.wm.cs.cs301.DavidNi.generation.Floorplan;
import edu.wm.cs.cs301.DavidNi.generation.Wall;
...
public class FirstPersonView {
	...
	/**
	 * Maze panel reference for drawing.
	 */
	...
	public void draw(MazePanel panel, int x, int y, int walkStep, int ang, float percentToExit) {
        ...
        // set the panel reference to the given panel
        mPanel = panel;
	...
	private boolean drawPolygons(int x1, int x2, int y11, int y12, int y21, int y22) {
		...
		while (x1i <= x2) {
			...
			mPanel.addFilledPolygon(xps, yps, 4);
